...
<Callout emoji='💡'>
  Think of state as a component's private data storage that can be modified over
  time.
</Callout>
...
<Callout emoji='🔑'>
  Use state for data that changes over time and props for data that's passed
  down from parent components.
</Callout>
...
<Callout emoji='🚀'>
  Practice working with state by building small, interactive components. Try
  creating a form with multiple fields, a todo list, or a simple game to
